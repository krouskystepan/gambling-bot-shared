...
export const BLACKJACK_IDLE_CLOSE_DAYS = 1
...
export const blackjackIdleCloseMs = (): number =>
  BLACKJACK_IDLE_CLOSE_DAYS * DAY_MS
...
export const hoursUntilBlackjackIdleClose = (
  updatedAt: Date,
  nowMs: number = Date.now()
): number => {
  const closeAt = updatedAt.getTime() + blackjackIdleCloseMs()
